...
function renderSubtasksEditDialog_addTask() {
    ...
    container.innerHTML = subtaskListEditHTML_addTask();
    added_subtasks_edit_addTask();
}

function subtaskListEditHTML_addTask() {
    return /*html*/`
        ...
        <div class="unordered-list_subtasks">
            <ul id="added_subtasks_edit_addTask"></ul>
        </div>
    `;
}

function added_subtasks_edit_addTask() {
    let container = document.getElementById('added_subtasks_edit_addTask');
    container.innerHTML = '';
    for (let i = 0; i < new_subtask_addTask_dialog.length; i++) {
        let subtask = new_subtask_addTask_dialog[i];
        container.innerHTML += subtasklistEditHTML_addTask(subtask, i);
    }
}

function subtasklistEditHTML_addTask(subtask, subtaskId) {
    return `
            <div id="subtask_list_row_main_addTask${subtaskId}" class="subtask_list_row_main">
                <div class="subtask_list_row" ondblclick="editSubtask('${subtask}', ${subtaskId})">
                    <div id="subtask_name_edit_id_addTask${subtaskId}" class="subtask_name_edit"><span class="point">&bull; </span> ${subtask}</div>
                        <div class="subtask_delete_and_edit_editDialog">
                        <a class="edit_btn_subtask" id="edit_img_subtask_edit_addTask${subtaskId}" onclick="editSubtask('${subtask}', ${subtaskId})">
                        ...
                        <a class="delete_btn_subtask" onclick="deleteSubtask(${subtaskId})">
...
function editSubtask_addTask(subtask, subtaskId) {
    ...
    container.innerHTML = editSubtaskHTML_addTask(subtask);
}

function editSubtaskHTML_addTask(subtask) {
    return `
...
function deleteSubtask_addTask(subtaskId){
    ...
    renderSubtasksEditDialog_addTask(taskId);
}
